...
export class DmService {
  ...
  async generateAndPublishKeys(): Promise<void> {
    ...
    const txXdr = this.client.publishDmKey(this.userAddress, this.keypair.publicKey);
  ...
  async getMessages(
    otherAddress: string
  ): Promise<Array<ConversationMessage & { content: string }>> {
    try {
  ...
  onRealTimeEvent(listener: (payload: Record<string, unknown>) => void) {
